...
void	init_ray(t_game *game, t_draw *draw, int x)
{
	...
	{
		draw->stepX = 1;
		draw->sideDistX = (draw->mapX + 1.0 -  game->p1.posx) * draw->deltaDistX;
	}
	if (draw->rayDirY < 0) 
	{
		draw->stepY = -1;
	...
	{
		draw->stepY = 1;
		draw->sideDistY = (draw->mapY + 1.0 -  game->p1.posy) * draw->deltaDistY;
	}
}
...
void	wall_dist(t_game *game, t_draw *draw, int x)
{
	...
	draw->hit = 0; // was a NS or a EW wall hit?
	while (!draw->hit) 
		wall_hit(game, draw);
	if (draw->side == 0)
